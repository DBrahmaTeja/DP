...
In the worst case, we are traversing the whole string to maintain the DP list and store the index in the map. Therefore, overall time complexity will be O(N).

Space Complexity
O(N), where ‘N’ is the size of the string.

In the worst case, we are using an extra list of size N+1 as well as an extra map. So, overall space complexity will be O(N).
*/
/*
    Time complexity: O(N)
